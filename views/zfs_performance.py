"""
ZFS Performance Views
Provides web interface for real-time ZFS performance monitoring
"""
...
router = APIRouter(prefix="/zfs/performance", tags=["zfs-performance"], dependencies=[Depends(get_current_user)])
performance_service = ZFSPerformanceService()


@router.get("/", response_class=HTMLResponse)
async def performance_index(request: Request):
    """Main performance dashboard with live monitoring overview"""
    try:
        # Get all processes for count, but filter display to >1% CPU
        ...
        capacity_stats = performance_service.get_pool_capacity_stats()
        
        # Get a quick ARC snapshot
        arc_stats = performance_service._read_arc_stats()
        
        return templates.TemplateResponse(
            "zfs/performance/index.jinja",
            {
                "request": request,
                ...
                "arc_stats": arc_stats,
                "page_title": "ZFS Performance Monitor"
            }
        )
...
async def pool_iostat_page(
    ...
    try:
        ...
        return templates.TemplateResponse(
            "zfs/performance/pool_iostat.jinja",
            {
                "request": request,
...
async def pool_iostat_partial(
    ...
    try:
        ...
        for stat in statistics:
            ...
            # Add visual indicator for hierarchical structure
            device_prefix = ''
            if indent_level > 0:
...
async def zfs_processes_page(request: Request, interval: int = 5):
    ...
    try:
        # Show all processes sorted by CPU% descending
        processes = performance_service.get_zfs_processes(sort_by_cpu=True)
        
        return templates.TemplateResponse(
            "zfs/performance/processes.jinja",
            {
                "request": request,
                "processes": processes,
                "interval": interval,
...
async def pool_capacity_page(
    request: Request,
    pool: Optional[str] = None
):
    """Display pool capacity statistics"""
    try:
        from services.zfs_pool import ZFSPoolService
        pool_service = ZFSPoolService()
        ...
        # Get capacity stats
        capacity_stats = performance_service.get_pool_capacity_stats(pool_name=pool)
        
        return templates.TemplateResponse(
            "zfs/performance/capacity.jinja",
            {
                "request": request,
                "all_pools": pool_names,
                "selected_pool": pool,
                "capacity_stats": capacity_stats,
                "page_title": "Pool Capacity Statistics"
            }
        )
    except Exception as e:
        return templates.TemplateResponse(
...
async def capacity_api(pool: Optional[str] = None):
    """API endpoint for fetching live capacity stats"""
    try:
        capacity_stats = performance_service.get_pool_capacity_stats(pool_name=pool)
        return JSONResponse(content=capacity_stats)
    except Exception as e:
        return JSONResponse(
            content={"error": str(e)},
            status_code=500
...
async def dataset_space_page(
    ...
    try:
        ...
        return templates.TemplateResponse(
            "zfs/performance/dataset_space.jinja",
            {
                "request": request,
...
async def vdev_stats_page(
    ...
    try:
        ...
        return templates.TemplateResponse(
            "zfs/performance/vdev_stats.jinja",
            {
                "request": request,
                "all_pools": pool_names,
...
async def system_iostat_page(
    ...
    try:
        ...
        return templates.TemplateResponse(
            "zfs/performance/system_iostat.jinja",
            {
                "request": request,
                "system": system,
                "interval": interval,
